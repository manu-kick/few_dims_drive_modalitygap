...
def _save_npz(
    ...
    np.savez_compressed(
        ...
        label_names=label_names.astype(object),
...
def precompute_coco_split(
    *,
    ...
    def flush():
        ...
        print(
            f"[SAVED] {out_path} | "
            f"N={len(img_ids_np)} | vision={vis_np.shape} | text={text_np.shape}"
        )
    ...
    for img_id in tqdm(img_ids, desc=f"COCO {split_name}"):
        ...
        unique_cats = sorted(set(a["category_id"] for a in anns))

        if len(unique_cats) == 1:
            label_id = int(unique_cats[0])
            label_name = cat_id_to_name[label_id]
        else:
            label_id = -1
            label_name = "multi_object"

        if single_object_only and label_id == -1:
            continue
...
def main():
    ...
    # scegli: "train" oppure "val"
    SPLIT = "train"

    DEVICE = "cuda:3" if torch.cuda.is_available() else "cpu"

    NORMALIZE = True
    SHARD_SIZE = 2048           # None = un unico shard gigante
    TEXT_MODE = "first_caption" # "first_caption" oppure "all_captions"
    SINGLE_OBJECT_ONLY = False

    CLIP_MODEL = "ViT-B-32"
    CLIP_PRETRAINED = "laion2b_s34b_b79k"

    # output separato per split
